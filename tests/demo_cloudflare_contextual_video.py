"""Combines: NarrativeArc (Step E, local, zero-AI-vendor structure)
  + CloudflareSceneRenderer (real contextual photos per scene, replacing
    the local HTML icon cards)
  + EdgeTtsProvider (real measured narration duration, unchanged from Step E)
  + FfmpegVideoRenderer (Ken-Burns motion + real transitions, unchanged
    from Step E)
into one video whose visuals actually match the document's subject
matter, not generic cards. English only. No avatar yet (separate,
still-blocked D-ID/Hedra issue).

Run: PYTHONPATH=backend .venv/bin/python tests/demo_cloudflare_contextual_video.py
"""
...
PROJECT_ID = "cloudflare-contextual-demo"
...
def main() -> None:
    ...
    for scene in scenes:
        ...
        print(f"[{scene.narrative_role.value:12s}] prompt: {prompt}")
        image_asset = renderer.render_scene(scene)
        print(f"  -> {image_asset.provider_name}: {image_asset.storage_path}")
        ...
        print(f"  audio: {audio_asset.duration_seconds:.2f}s\n")
